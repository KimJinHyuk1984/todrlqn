...
def main():
    ...
    st.error("이 사이트에서 제공되는 결과물은 생기부 기록을 위한 초안입니다. 최종 기록은 반드시 교사의 검토 후 작성해주세요.")
    ...
    if file is not None:
        ...
        # --- 열 이름 기준으로 특정 열 삭제 (수정된 부분) ---
        # 삭제할 열 이름 리스트
        columns_to_drop = [
            '순번', '아이디', '학번', '필명', '글내용(통합)', '교사평가', 
            '글번호', '투표', '득표', '1차점수', '점수', '조회', 
            '첨부', '제출일', '최종수정일', 'IP'
        ]
        # 지정된 열 삭제 (해당 열이 없는 경우 오류 발생 방지)
        new_df = df.drop(columns=columns_to_drop, errors='ignore')

        st.warning("다음 열들을 제외하고 남은 데이터로 작업을 진행합니다: [순번, 아이디, 학번, 필명, 글내용(통합), 교사평가, 글번호, 투표, 득표, 1차점수, 점수, 조회, 첨부, 제출일, 최종수정일, IP]")
        st.dataframe(new_df)

        byte = st.text_input("생성할 특기사항의 글자 수를 byte단위로 입력해주세요. (예: 500)", key="byte_input")
        ...
        st.markdown("초안 작성에 활용되지 않을 부분은 [**글 작성시 참고하지 않음**]의 형태로 작성해주세요.")

        # new_df의 2번째 열부터 시작 (일반적으로 이름 다음부터의 항목)
        # 이 부분은 이름, 학번 등 식별자 열이 앞쪽에 있다는 가정하에 작성되었습니다.
        # 만약 식별자 열이 하나만 있다면 range(1, len(new_df.columns))로 조정해야 할 수 있습니다.
        ...
        if st.button("✨ 특기사항 생성하기"):
            ...
            if not api_key:
                ...
                with st.spinner('AI가 학생별 요약문을 생성 중입니다. 잠시만 기다려주세요...'):
                    # 항목 설명 텍스트 생성 (이름 열 제외)
                    explanation_text = "\n".join([
                        f"- {col} : {desc}" for col, desc in zip(new_df.columns[1:], descriptions) if desc.strip()
                    ])
                    ...
                    for idx, row in new_df.iterrows():
                        student_name = row[new_df.columns[0]] # 학생 이름 또는 첫 번째 열의 값
                        
                        # 학생 입력값 텍스트 생성 (이름 열 제외)
                        student_inputs = "\n".join([
                            f"{col} : {row[col]}" for col in new_df.columns[1:]
                        ])
                        ...
                        **Instructions**:
                        ...
                        - "글 작성시 참고하지 않음"이라고 입력된 부분은 AI가 참고하지 않습니다.
